...
old_section = re.search(r'\{/\* Blood Bank Stock \*/\}[\s\S]*?\}\s*\}\)', src)
if old_section:
    new_section = """{/* Blood Bank Stock */}
      {bloodStock.length > 0 && (
                  ...
                  {canEdit && (
                    <div style={{ display: 'flex', gap: 4 }}>
                      <button
                        ...
                        disabled={isEmpty}
                        style={{ flex: 1, padding: '4px 0', fontSize: 12, borderRadius: 6, background: isEmpty ? 'transparent' : 'rgba(239,68,68,0.1)', color: isEmpty ? '#9ca3af' : '#ef4444', border: '1px solid ' + (isEmpty ? '#e5e7eb' : '#ef4444'), cursor: isEmpty ? 'not-allowed' : 'pointer' }}
                      >− Dispatch</button>
                      <button
                        onClick={() => updateBloodStock(s.blood_group, 1)}
                        style={{ flex: 1, padding: '4px 0', fontSize: 12, borderRadius: 6, background: 'rgba(34,197,94,0.1)', color: '#22c55e', border: '1px solid #22c55e', cursor: 'pointer' }}
                      >+ Receive</button>
                    </div>
                  )}
                </div>
    ...
    src = src[:old_section.start()] + new_section + src[old_section.end():]
    print('Blood stock section redesigned')
else:
