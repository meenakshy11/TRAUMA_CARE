# Fix authStore - clears old keys, proper logout
...
# Fix .env - disable demo mode so real credential checking works
with open("/workspace/apps/web/.env", "r") as f:
    env = f.read()
env = env.replace("VITE_DEMO_MODE=true", "VITE_DEMO_MODE=false")
with open("/workspace/apps/web/.env", "w") as f:
    f.write(env)
print("DEMO_MODE=false set in .env")

# Fix api/index.ts base URL
...
# Fix DEMO constant to use new env var
if "const DEMO = import.meta.env.VITE_DEMO_MODE" not in api:
    api = 'const DEMO = import.meta.env.VITE_DEMO_MODE === "true"\n' + api

with open("/workspace/apps/web/src/api/index.ts", "w") as f:
    f.write(api)
print("api/index.ts fixed")

# Fix demo-fixtures to have proper DEMO_USERS
with open("/workspace/apps/web/src/api/demo-fixtures.ts", "w") as f:
    f.write("""export const DEMO_USERS: Record<string, { password: string; user: any; tokens: any }> = {
