...
# Remove everything from blood section onwards
idx = src.find('\n        {/* Blood Bank Stock */}')
if idx != -1:
    src = src[:idx]
    print('Removed old blood section')

# The clean file ends with:  </div>\n  
# We need to add blood section then close properly
blood_section = """
        {/* Blood Bank Stock */}
        {bloodStock.length > 0 && (
...
# Remove the incomplete ending and add proper closing
src = src.rstrip()
# Remove trailing incomplete closing tags
while src.endswith('\n  ') or src.endswith('  '):
    src = src.rstrip()

src = src + blood_section

p.write_text(src, encoding='utf-8')
print('Done')
print('ends with:', repr(src[-50:]))
